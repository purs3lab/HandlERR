...
#include "clang/CConv/CtxSensAVarBounds.h"
...
insertCtxSensBoundsKey(ProgramVar *OldPV,
  ...
  ABI->ProgVarGraph.addEdge(OldPV->getKey(), NKVar->getKey());
  ABI->CtxSensProgVarGraph.addEdge(NKVar->getKey(), OldPV->getKey());
}
...
bool CtxSensitiveBoundsKeyHandler::tryGetFieldCSKey(MemberExpr *ME,
                                                    ASTContext *C,
                                                    ProgramInfo &I,
                                                    BoundsKey &CSKey) {
  bool RetVal = false;
  ...
  if (FD != nullptr) {
    ...
    if (MECSMap->find(AK) != MECSMap->end()) {
    ...
    if (RD != nullptr) {
      ...
      for (auto *CFD : RD->fields()) {
        ...
        if (auto *SS =
          ...
          CSS = CtxStructScope::getCtxStructScope(SS, AK, SKV.IsGlobal);
        }
        createCtxSensBoundsKey(MEBKey, CSS, BKeyMap);
      }
    }
  }
}
