...
CSetBkeyPair
  ...
  if (E != nullptr) {
    ...
    if (TypE->isRecordType() || TypE->isArithmeticType()) {
      ...
      return std::make_pair(pvConstraintFromType(TypE), ABI.getCtxSensFieldBoundsKey(E, Context, Info));
    ...
    if (ExplicitCastExpr *ECE = dyn_cast<ExplicitCastExpr>(E)) {
      ...
      if (!isNULLExpression(ECE, *Context) && TypE->isPointerType() &&
        ...
        constrainConsVarGeq(Vars, Ret.first, CS, nullptr, Safe_to_Wild, false, &Info);
