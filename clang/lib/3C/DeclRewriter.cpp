...
void DeclRewriter::rewriteMultiDecl(MultiDeclInfo &MDI, RSet &ToRewrite) {
  ...
  for (auto MIt = MDI.Members.begin(); MIt != MDI.Members.end(); MIt++) {
    ...
    // multi-decl members as opposed to function parameters and returns (TODO:
    // file an issue?), but we are probably still better off overwriting the
    // annotations with a complete, valid declaration than mixing and matching a
    // declaration generated by 3C with existing annotations.
    //
