  ...
  for (const auto &V : Info.getVarMap()) {
    ...
    if (Decl *D = PSLMap[PLoc]) {
      ...
      if (PVChanged && !PV->isPartOfFunctionPrototype()) {
        ...
        std::string ReplacementText = buildDeclVar(MMD, PV, Context, Info);
...
void DeclRewriter::rewriteMultiDecl(MultiDeclInfo &MDI, RSet &ToRewrite) {
  ...
  for (auto It = MDI.Members.begin(); It != MDI.Members.end(); It++) {
    ...
    if (IsFirst) {
      ...
      IsFirst = false;
      if (Found) {
        SourceRange SR = SameLineReplacement->getSourceRange(SM);
      ...
      if (Found) {
        ...
        // TODO: Can we factor out any of this logic with doDeclRewrite?
        ...
        // We do want to overwrite existing Checked C annotations. 3C currently
        // doesn't have real itype support for multi-decl members as opposed to
        // function parameters and returns (TODO: file an issue?), but we are
        // probably still better off overwriting the annotations with a
        // complete, valid declaration than mixing and matching a declaration
        // generated by 3C with existing annotations.
        //
        // Code copied from DeclReplacement::getSourceRange.
        // REVIEW: Can the code duplication be reduced without making a bigger
        // mess?
        SourceLocation AnnotationsEnd = getCheckedCAnnotationsEnd(DL);
        if (AnnotationsEnd.isValid() &&
            (!EndLoc.isValid() || SM.isBeforeInTranslationUnit(EndLoc, AnnotationsEnd)))
          EndLoc = AnnotationsEnd;

        std::string NewDeclStr;
        if (MDI.BaseTypeRenamed) {
          // In this case, DL->print wouldn't give us the new base type name, so
          // we use mkString, even though it has bugs in some cases. An
          // alternative would be to try to combine the new base type name with
          // the original source for the rest of the declaration, but that may
          // run into problems with macros and the like, so we would still need
          // this as a fallback.
          NewDeclStr = mkStringForDeclWithUnchangedType(DL, A, Info);
        } else {
          ...
          DL->print(DeclStream);
          NewDeclStr = DeclStream.str();
          assert("Original decl string empty." && !NewDeclStr.empty());
    ...
    if (!IsLast) {
      // Same as in the "type hasn't changed" case above, except we _do_ want to
      // skip past an initializer if there is one.
      // REVIEW: Can the logic be combined and simplified?
      SourceLocation EndLoc = DL->getEndLoc();
      SourceLocation AnnotationsEnd = getCheckedCAnnotationsEnd(DL);
      if (AnnotationsEnd.isValid() &&
          (!EndLoc.isValid() || SM.isBeforeInTranslationUnit(EndLoc, AnnotationsEnd)))
        EndLoc = AnnotationsEnd;
      SourceRange Comma = getNextComma(EndLoc);
